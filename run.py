...
#   src = eh.get_top_right_node()
#   eh.append_random_demand(NUM_STOPS, cluster_num=0, cluster_jump=0, dron_min_w=0,
...
#   # --------------------------------
#   print("Set", 2, "for min weight", 0.5, ", 20% drone loading, and drone speed", 20, "m/s begins!")
#   init_globals(max_truck_speed=12, base_truck_speed=1.4, truck_city_mpg=24,
#               base_temperature=TEMPERATURE, temp_flucts_coeff=3, drone_speed=20,
#               relative_humidity=RH(isMorning,GET_MONTH_INDEX[Month]))
#   eh = EnergyHelper.load("pickles/manhattan-policy-set-{}-{}ms.pkl".format(2, 20))
...
#   src = eh.get_top_right_node()
#   eh.append_random_demand(NUM_STOPS, cluster_num=0, cluster_jump=0, dron_min_w=0.5,
#                           drone_only_possible_component=0.2)
#   conflict = src in [x[0] for x in eh.demand]
#   while conflict:
#     eh.reset_demand()
...
#   print(swp)
#   # --------------------------------  
#   print("Set", 2, "for min weight", 0.5, ", 20% drone loading, and drone speed", 20, "m/s has ended.")
...
#         # Loading Manhattan Set + Generating Demand, Source
#         # --------------------------------
#         print("Set", 1, "for min weight", min_w, ", 20% drone loading, and drone speed", V, "m/s begins!")
#         init_globals(max_truck_speed=12, base_truck_speed=1.4, truck_city_mpg=24,
...
#         print("Set", 1, "for min weight", min_w, ", 20% drone loading, and drone speed", V, "m/s has ended.")

#   for min_w in [0.0, 0.5, 1.0, 1.5, 2.0]:
#       for V in [10, 20]:
#         # --------------------------------
#         # Loading Manhattan Set + Generating Demand, Source
#         # --------------------------------
...
#           # Loading Manhattan Set + Generating Demand, Source
#           # --------------------------------
#           print("Set", 2, "for min weight", min_w, ", ", ld, "% drone loading, and drone speed", V, "m/s begins!")
#           init_globals(max_truck_speed=12, base_truck_speed=1.4, truck_city_mpg=24,
#                       base_temperature=TEMPERATURE, temp_flucts_coeff=3, drone_speed=V,
#                       relative_humidity=RH(isMorning,GET_MONTH_INDEX[Month]))
#           eh = EnergyHelper.load("pickles/manhattan-policy-set-{}-{}ms.pkl".format(2, V))
#           print(eh.edges[:5])  # confirming calibration
#           NUM_STOPS = 200
#           RANGE = float(3000)   # dummy for now
#           src = eh.get_top_right_node()
#           eh.append_random_demand(NUM_STOPS, cluster_num=0, cluster_jump=0, dron_min_w=min_w,
#                                   drone_only_possible_component=(ld / 100))
#           conflict = src in [x[0] for x in eh.demand]
#           while conflict:
#             eh.reset_demand()
#             eh.append_random_demand(NUM_STOPS, cluster_num=0, cluster_jump=0, dron_min_w=min_w,
...
#           print("Set", 2, "for min weight", min_w, ", ", ld, "% drone loading, and drone speed", V, "m/s has ended.")
...
#           # Loading Manhattan Set + Generating Demand, Source
#           # --------------------------------
#           print("Set", i, " spec for min weight", min_w, ", 20% drone loading, and drone speed", V, "m/s begins!")
#           init_globals(max_truck_speed=12, base_truck_speed=1.4, truck_city_mpg=24,
#                       base_temperature=TEMPERATURE, temp_flucts_coeff=3, drone_speed=V,
#                       relative_humidity=RH(isMorning,GET_MONTH_INDEX[Month]))
  ...
  for i in [2]:
    for min_w in [1.0]:
        for V in [20]:
          ...
          RANGE = float(3000)   # dummy for now
          src = eh.get_top_right_node()
          eh.add_demand_list([(2808, 2), (1014, 2.88), (1720, 2.78), (4099, 3.02), (1272, 2.81), (716, 1.5), (3331, 2.34), (2462, 2.47), (5305, 4.04), (3355, 2.44), (2303, 2), (860, 2.61), (4037, 3.28), (3881, 3.25), (725, 1), (4684, 3.47), (2206, 1.25), (3315, 2.27), (1853, 2.8), (1555, 3.57), (2086, 2.56), (2847, 3.09), (4449, 3.68), (252, 2.22), (90, 2.46), (2396, 1.25), (835, 2.71), (1833, 1.5), (212, 2.9), (4351, 1.75), (1814, 2.27), (3357, 3.13), (3732, 2), (3066, 2.99), (2252, 3.33), (4335, 4.18), (3904, 2.37), (1487, 3.62), (910, 3.93), (1424, 3.32), (5343, 2.76), (4251, 1.5), (4476, 2.88), (1284, 3.51), (913, 2.32), (927, 2.1), (3114, 2.63), (5542, 2.52), (3250, 2.33), (2118, 2.92), (1906, 3.98), (3905, 2.47), (972, 3.1), (917, 2.94), (1858, 1), (2833, 1.5), (3998, 3.83), (834, 2.26), (4148, 2.18), (346, 2.16), (4175, 3.69), (1184, 3.56), (3812, 1.5), (3655, 1.75), (3189, 2.48), (629, 3.11), (1565, 3.74), (5054, 1), (3767, 2.16), (3703, 1), (2496, 3.89), (1084, 2.77), (1179, 2.88), (1901, 3.44), (2515, 3.57), (4774, 2.53), (3204, 2.51), (3393, 3.63), (1483, 3.56), (933, 1.75), (3558, 1), (4672, 1.5), (2796, 1.25), (3566, 3.53), (452, 3.47), (1494, 2.61), (1245, 2.66), (3132, 4.09), (5758, 4.1), (3498, 4.12), (1619, 1.75), (2247, 3.47), (3354, 3.29), (3075, 2.74), (1957, 3.27), (0, 1.25), (269, 3.73), (1817, 2.7), (181, 2.82), (4287, 1.5), (3312, 3.27), (501, 1.75), (2928, 2.32), (675, 3.92), (2513, 2), (6053, 3.35), (829, 3.46), (2537, 3.61), (4164, 3.74), (1935, 3.74), (2458, 3.74), (1077, 2.51), (1633, 2.24), (4802, 3.35), (3427, 4.13), (964, 1.75), (3082, 3.98), (1334, 3.14), (2780, 3.11), (2945, 2.79), (2629, 4.14), (5009, 3.75), (2693, 2.5), (3229, 2.1), (3867, 3.94), (3012, 3.74), (1959, 3.61), (2840, 2.21), (6019, 2.26), (444, 2), (1053, 3.24), (785, 2.2), (1097, 3.6), (2961, 3.55), (1889, 2.55), (1791, 3.44), (2236, 3.78), (1991, 3.27), (879, 2.47), (743, 2.34), (93, 3.55), (4033, 3.87), (5019, 2.69), (740, 2), (4160, 4.17), (5243, 3.24), (4129, 1), (926, 3.35), (4338, 2), (1010, 2.64), (3608, 4.14), (2285, 3.55), (2497, 2.14), (1755, 2.83), (6199, 2.72), (1664, 1.75), (2628, 3.7), (5031, 3.8), (5462, 3.82), (744, 2.8), (3181, 2.18), (4504, 3.91), (3764, 4.15), (3347, 2.72), (5074, 3.13), (3678, 3.1), (2935, 3.97), (2359, 2.16), (2077, 1.5), (3400, 2.89), (2318, 3.92), (4169, 3.17), (4683, 1.5), (924, 1.5), (3, 3.22), (3792, 3.3), (4356, 3.21), (5411, 2.17), (345, 2), (3224, 3.57), (2599, 1), (2960, 3.95), (1454, 2.83), (1279, 1.75), (6048, 3.33), (1121, 3.77), (3839, 3.27), (3930, 3.39), (1131, 3.43), (3612, 1), (1103, 3.53), (2372, 2.16), (1290, 2), (4286, 2.85), (1019, 3.98), (546, 3.63), (1348, 3.69), (4362, 3.93), (3578, 1), (1880, 2.14)])
          print("Source", src, "at", eh.nodes[src])
          print(eh.demand)
          # --------------------------------
...
#         # Loading Manhattan Set + Generating Demand, Source
#         # --------------------------------
#         print("Set", i, "for min weight", 0, ", 20% drone loading, and drone speed", V, "m/s begins!")
#         init_globals(max_truck_speed=12, base_truck_speed=1.4, truck_city_mpg=24,
#                     base_temperature=TEMPERATURE, temp_flucts_coeff=3, drone_speed=V,
#                     relative_humidity=RH(isMorning,GET_MONTH_INDEX[Month]))
#         eh = EnergyHelper.load("pickles/manhattan-policy-set-{}-{}ms.pkl".format(i, V))
...
#         print("Set", i, "for min weight", 0, ", 20% drone loading, and drone speed", V, "m/s has ended.")
  
#   print("Set", 2, "for min weight", 0, " and drone speed", 20, "m/s begins!")
#   init_globals(max_truck_speed=12, base_truck_speed=1.4, truck_city_mpg=24,
#               base_temperature=TEMPERATURE, temp_flucts_coeff=3, drone_speed=20,
#               relative_humidity=RH(isMorning,GET_MONTH_INDEX[Month]))
#   eh = EnergyHelper.load("pickles/manhattan-policy-set-{}-{}ms.pkl".format(2, 20))
...
#   eh.append_random_demand(NUM_STOPS, cluster_num=0, cluster_jump=0, dron_min_w=0, src=src,
#                           drone_only_possible_component=0.2)
#   conflict = src in [x[0] for x in eh.demand]
#   while conflict:
#     eh.reset_demand()
...
#   print(swp)
#   # --------------------------------  
#   print("Set", 2, "for min weight", 0, " and drone speed", 20, "m/s has ended.")

  for min_w in range(15, 11, 5):
    for i in range(2, 3):
      ...
      #   src = eh.get_close_node(-3302, -3509, 20)
        ...
        eh.append_random_demand(NUM_STOPS, cluster_num=0, cluster_jump=0, dron_min_w=(min_w / 10),
                                src=src, drone_only_possible_component=0.2)
        conflict = src in [x[0] for x in eh.demand]
        while conflict:
          eh.reset_demand()
          eh.append_random_demand(NUM_STOPS, cluster_num=0, cluster_jump=0, dron_min_w=(min_w / 10),
